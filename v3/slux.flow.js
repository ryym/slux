declare module "./slux" {
  ...
  declare type MutationFunc<S, P> = (state: S, payload: P) => S;

  // FullState, State, P
  declare interface _Mutation<FS, S, P> {
    type: string;
  ...
  declare type Mutation<S, P> = _Mutation<S, S, P>
  declare type CustomMutation<FS, S, P> = _Mutation<FS, S, P>

  declare type MutationCreator<FS, S> = <P>(
    type: string,
    mutationFunc: MutationFunc<S, P>
  ) => _Mutation<FS, S, P>

  declare function mutation<S, P>(
    type: string,
    mutationFunc: MutationFunc<S, P>
  ): Mutation<S, P>

  declare function createMutation<FS, S>(
    selectState: FS => S,
    mergeState: (FS, S) => FS
  ): MutationCreator<FS, S>

  declare type Commit<S> = <P>(mutation: _Mutation<S, any, P>, payload: P) => void;


  declare type ActionFunc<S, P, R> = (store: Store<S, any>, state: S, payload: P) => R;
  declare type Run<S> = <P, R>(action: Action<S, P, R>, payload: P) => R;

  declare interface Action<S ,P, R> {
    type: string;
    exec: ActionFunc<S, P, R>;
    // (commit: Commit<S>, state: S, payload: P): R;
  }

  declare function action<S, P, R>(
    type: string,
    actionFunc: ActionFunc<S, P, R>
  ): Action<S, P, R>


  declare interface StoreConfig<State, Snap> {
    getInitialState: () => State;
    takeSnapshot: State => Snap;
  }
  declare class Store<State, Snap> {
    constructor(name: string, config: StoreConfig<State, Snap>): Store<State, Snap>;

    commit: <P>(mutation: _Mutation<State, any, P>, payload: P) => void;
    run: <P, R>(action: Action<State, P, R>, payload: P) => R;
  ...
  declare function createStore<State, Snap>(name: string, config: StoreConfig<State, Snap>): Store<State, Snap>


  declare interface Command<P> {
    (payload: P): { type: string, payload: P }
  }
  declare interface DefineCommand<State> {
    <P>(mutation: _Mutation<State, any, P>): Command<P>;
    <P>(action: Action<State, P, any>): Command<P>;
  }
  declare class Dispatcher<State> {
    define: DefineCommand<State>;
    dispatch<P>(command: Command<P>, payload: P): void;
    // define<P>(mutation: _Mutation<State, any, P>): Command<P>;
    // define<P>(action: Action<State, P, any>): Command<P>;
  }

  declare function createDispatcherWithCommands<State, Commands>(
